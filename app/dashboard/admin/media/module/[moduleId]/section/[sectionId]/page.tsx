...
export default async function Page(props: z.infer<typeof propsSchema>) {
    ...
    const [modulesResult, sectionsResult, allVideosResult] = await Promise.all([getModuleById(props.params.moduleId), getSectionsForModule(props.params.moduleId), getVideosForSection(safeProps.params.sectionId)]);
    ...
    return <AdminSectionsManageClientWrapper module={module} section={section}>
        <div className="grid grid-rows-[auto_1fr] w-[95%] h-full max-w-[50rem] mt-4 mx-auto">
            ...
            <AdminSectionManageVideosTable moduleId={safeProps.params.moduleId} sectionId={safeProps.params.sectionId} allSections={allSections} allVideos={allVideosResult.value} />
        </div>
