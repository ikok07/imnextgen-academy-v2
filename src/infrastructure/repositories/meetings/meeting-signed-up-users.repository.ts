...
export class MeetingSignedUpUsersRepository extends BaseRepository implements IMeetingSignedUpUsersRepository {
    ...
    removeSignedUpUser(opts: RemoveSignedUpUserOptions): Promise<void> {
        try {
            return this.queryDB(async db => {
                if (opts.userId) {
                    await db.delete(meetingSignedUpUsersTable).where(and(eq(meetingSignedUpUsersTable.profile_id, opts.userId), eq(meetingSignedUpUsersTable.meeting_id, opts.meeting_id), eq(meetingSignedUpUsersTable.meeting_start_date, Math.floor(opts.start_date))));
                } else {
                    await db.delete(meetingSignedUpUsersTable).where(and(eq(meetingSignedUpUsersTable.email, opts.email!), eq(meetingSignedUpUsersTable.meeting_id, opts.meeting_id), eq(meetingSignedUpUsersTable.meeting_start_date, Math.floor(opts.start_date))));
                }
            })
        } catch (e) {
            throw new DatabaseError(`Failed to get signed up users for meeting: ${e}`);
